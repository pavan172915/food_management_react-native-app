...
export const fetchProducts = () => {
  return async (dispatch) => {
    ...
    try {
      ...
      dispatch({ type: SET_PRODUCTS, products: loadedProductsFromDB });
    } catch (err) {
      throw err;
    }
  };
...
export const deleteProduct = (productId) => {
  return async (dispatch) => {
    const responseData =  await fetch(`${fireBaseUrl}/products/${productId}.json`, {
      method: "DELETE",
    });
    if(!responseData.ok){
        throw new Error('Something Went wrong !!')
...
export const updateProduct = (id, title, description, imageURL) => {
  return async (dispatch) => {
    const responseData = await fetch(`${fireBaseUrl}/products/${id}.json`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
      },
